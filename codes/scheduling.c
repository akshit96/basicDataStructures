#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <stdbool.h>
#include <assert.h>
#include <ctype.h>

#define max(a,b) (a>b)?a:b
#define min(a,b) (a<b)?a:b

int counter = 1 ;
typedef struct node
{
	int i,r,c,p ;
	struct node *lc,*rc,*parent ;
}node ;
typedef struct tree
{
	node* root ;
}tree ;
node* newnode(int i,int r,int c,int p)
{
	node* ret = (node*)malloc(sizeof(node)) ;
	ret->i = i ;
	ret->r = r ;
	ret->c = c ;
	ret->p = p ;
	ret->lc = ret->rc = ret->parent = NULL ;
	return ret ;
}
node* treeMax(node* x)
{
	node *temp = x ;
	if(temp == NULL)
		return NULL ;
	while(temp->rc != NULL)
	{
		temp = temp->rc ;
	}
	return temp ;
}
node* treeMin(node* x)
{
	node *temp = x ;
	if(temp == NULL)
		return NULL ;
	while(temp->lc != NULL)
	{
		temp = temp->lc ;
	}
	return temp ;
}
node* treeSearch(node* x,int k)
{
	if(x == NULL || x->p == k)
		return x ;
	if(k < x->p)
		return (treeSearch(x->lc , k)) ;
	else
		return (treeSearch(x->rc , k)) ;
}
node* successor(node* x)// next node that appears in the inorder traversal of BST
{
	if(x == NULL)
		return x ;
	if(x->rc != NULL) // if x has right sub tree
	{
		return (treeMin(x->rc)) ;
	}
	else // visualize inorder traversal and get the idea (if x doesnt have a right subtree )
	{
		node* temp = x->parent ;
		while(temp != NULL && temp->rc == x)
		{
			x = temp ;
			temp = temp->parent ;
		}
		return temp ;
	}
}
node* treeInsertRecursive(node* root,node* new_node,node* par)
{
	if(root == NULL)
	{
		new_node->parent = par ;	
		return new_node; 
	}
	if(new_node->p < root->p)
	{
		root->lc = treeInsertRecursive(root->lc , new_node ,root) ;
	}
	else
	{
		root->rc = treeInsertRecursive(root->rc , new_node , root) ; 
	}
}
void treeInsert(tree* T,node* new_node)
{
	if(T->root == NULL) // empty tree
	{
		T->root = new_node ;
		return ;
	}
	node* cur = T->root ;
	node* par = NULL;
	while(cur != NULL)
	{
		par = cur ;
		if(new_node->p < cur->p)
		{
			cur = cur->lc ;
		}
		else if(new_node->p > cur->p)
		{
			cur = cur->rc ;
		}
		else
		{		
			if(new_node->r < cur->r)
			{
				cur = cur->rc ;	
			}
			else
			{
				cur = cur->lc ;
			}
		}
	}
	
	if(new_node->p < par->p)
	{
		par->lc = new_node ;
		new_node->parent = par ;
	}
	else if(new_node->p > par->p)
	{
		par->rc = new_node ;
		new_node->parent = par ;
	}
	else
	{
		if(new_node->r < cur->r)
		{
			par->rc = new_node ;
			new_node->parent = par ; 
		}
		else
		{
			par->lc = new_node ;
			new_node->parent = par ;
		}
	}
}
void transplant(tree* T ,node* u,node* v) // replaces Subtree rooted at u with subtree rooted at v
{
	if(u->parent == NULL)
	{
		T->root = v ;
	}
	else if( (u->parent)->lc == u)
	{
		(u->parent)->lc = v ;
	}
	else
	{
		(u->parent)->rc = v ;
	}
	if(v != NULL)
	{
		v->parent = u->parent ;
	}
}
void treeDelete(tree* T,node* z)
{
    if(z == NULL)
        return ;
	node* root = T->root ;
	if(z->lc == NULL)
	{
		transplant(T,z,z->rc) ;
	}
	else if(z->rc == NULL)
	{
		transplant(T,z,z->lc) ;
	}
	else // z has 2 children
	{
		node* y = successor(z) ; // surely have it as treemin(z->rc) as it has a right sub tree 
		if(y->parent != z) // if successor is not right child then adjust tree so as to make it right child
		{
			transplant(T,y,y->rc) ;
			y->rc = z->rc ;
			(y->rc)->parent = y ;
		}
		transplant(T,z,y) ;
		y->lc = z->lc ;
		(y->lc)->parent = y ;
	}
	z->lc = z->rc = z->parent = NULL ;
}
typedef struct Processor
{
	node* currentTask ;
}Processor ;
bool taken[1002] ;
int main()
{
	#ifndef ONLINE_JUDGE	
		freopen("input_dsa.txt" , "r" , stdin) ;
	#endif
	node* task[1002] ;
	int j = 0;
	int i=0,r,c,p ;
	int max_time = 0 ;
	while(~scanf("%d%d%d%d\n",&i,&r,&c,&p))
	{		
		task[++j] = newnode(i,r,c,p) ;
	}

	int  n = j ;
	max_time = task[n]->r ;
	Processor *processor = (Processor*)malloc(sizeof(Processor)) ;
	processor->currentTask = NULL ;
	
	tree* T = (tree* )malloc(sizeof(tree)) ;
	T->root = NULL ;
	j = 1 ;
	int t = 1 ;
	int processed = 0 ;
	while(t <= max_time || processor->currentTask != NULL)
	//while(processed < n)
	{
		// assert(0) ;
		while(j <= n && (task[j]->r <= t) )
		{
			// treeInsert(T,task[j]) ;
			T->root = treeInsertRecursive(T->root,task[j],NULL) ;
			j++ ;
		}
		
		if(processor->currentTask == NULL)
		{
			printf("0\n") ;
		}
		else
		{
			printf("%d\n",processor->currentTask->i) ;
			processor->currentTask->c -= 1 ;
			if(processor->currentTask->c == 0)
			{
				processed += 1 ;
				processor->currentTask = NULL ;
			}
		}
		node* temp = treeMax(T->root) ;
		if(temp != NULL)
		{
	    		if(processor->currentTask == NULL)
	    		{
	    		    processor->currentTask = temp ;
	    		    if(temp->parent == NULL)
	    		    {
	    		    	if(temp->lc != NULL)
	    		    	{
	    		    		temp->lc->parent = temp->parent ;
	    		    	}
	    		    	T->root = temp->lc ;
	    		    }
	    		    else
	    		    {
	    		    	temp->parent->rc = temp->lc ;
	    		    	if(temp->lc != NULL)
	    		    	{
	    		    		temp->lc->parent = temp->parent ; 
	    		    	}
	    		    }
	    		    temp->lc = temp->rc = temp->parent = NULL ;
	    		    // treeDelete(T,temp) ;
	    		}   
	    		else
	    		{
	    		    if(temp->p > processor->currentTask->p)
	    		    {
	    		        T->root = treeInsertRecursive(T->root,processor->currentTask,NULL) ;   		    		    processor->currentTask = temp ;
		    		    if(temp->parent == NULL)
		    		    {
		    		    	if(temp->lc != NULL)
		    		    	{
		    		    		temp->lc->parent = temp->parent ;
		    		    	}
		    		    	T->root = temp->lc ;
		    		    }
		    		    else
		    		    {
		    		    	temp->parent->rc = temp->lc ;
		    		    	if(temp->lc != NULL)
		    		    	{
		    		    		temp->lc->parent = temp->parent ; 
		    		    	}
		    		    }
		    		    temp->lc = temp->rc = temp->parent = NULL ;
	    		        // treeDelete(T,temp) ;
	    		        // processor->currentTask->lc = processor->currentTask->rc = processor->currentTask->parent = NULL ;
	    		    }
	    		}		    
		}
		t++ ;
	}
	return 0;
}
